...
// 1   KBCLK   8 (might need to swap with 9)
// 2   KBDATA  9 (might need to swap with 8)
// 3   KBRST   10
// 4   5v      5v
// 5   NC
// 6   GND     GND
            ...
            if (key==0x5F)  fn=keydown;  // "Help" key: special function on/off
              ...
              if (keydown){
                ...
                if (fn) {
                  ...
                  if      (key==0x50) keystroke(0x44,0);      // Help + F1 = F11
                  else if (key==0x51) keystroke(0x45,0);      // Help + F2 = F12
                  else if (key==0x5A) keystroke(0x53,0);      // Help + keypad [ = NumLock
                  else if (key==0x5B) keystroke(0x47,0);      // Help + keypad ] = ScrollLock
                  else if (key==0x02) keystroke(0x14,0x40);   // Help + 2 = @
                  else if (key==0x04) keystroke(0x35,0x02);   // Help + 4 = °
                  // MORE HELP FUNCTIONS
